...
const BUILD_VERSIONS = new Set([
  'archive-demo-nav GitHub Pages build v1\n',
...
function buildPages({ projectRoot = path.resolve(__dirname, '..'), outputDirectory, desktopUrl = '' } = {}) {
  ...
  if (fs.existsSync(destination)) {
    ...
    if (fs.lstatSync(destination).isSymbolicLink() || !fs.statSync(destination).isDirectory() ||
      ...
      !BUILD_VERSIONS.has(fs.readFileSync(marker, 'utf8'))) {
  ...
  fs.writeFileSync(path.join(destination, BUILD_MARKER), 'archive-demo-nav GitHub Pages build v2\n', { encoding: 'utf8', flag: 'wx' });
...
if (require.main === module) {
  try {
    ...
    console.info(result.remoteDesktopConfigured
      ? '已配置 HTTPS 远程桌面入口。'
      : '远程桌面未配置：系统入口会打开目标网址；密集架需要单位网络，艾搜客户端入口需在本机运行。');
  } catch (error) {
