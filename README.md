...
**无远程桌面的网页自动登录：** GitHub Pages 页面提供扩展 ZIP 下载和安装说明。每位使用者在自己的 Chrome/Edge 安装扩展一次，再为需要演示的网页系统分别保存密码；首页会显示扩展连接状态和已设置数量，卡片会标明密码是否已保存。之后从 Pages 点击系统演示，扩展可填写并提交 7 个网页系统的登录表单。密码不进入 GitHub。验证码和二次验证仍由使用者处理。
